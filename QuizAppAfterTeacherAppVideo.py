...
def makeQuestionTable(conn):
    code = '''CREATE TABLE IF NOT EXISTS QUESTIONS
            (ID INT PRIMARY KEY NOT NULL, QUESTION TEXT NOT NULL);'''
    conn.execute(code)
    conn.commit()


def setQuestionTable(conn, questions):
    questionID = 1
    for question in questions:
        code = "INSERT INTO QUESTIONS (ID, QUESTION) VALUES(?,?);"
        conn.execute(code, (questionID, question))
        questionID += 1
    conn.commit()
...
makeDBTable(conn)
makeQuestionTable(conn)
setQuestionTable(conn, questionList)
result= []
for a_question in questionList:
    result.append(None)

ID = newQuiz(conn, result)
